        ...
        <div className="border-x border-b border-scent-noir/10 bg-scent-parchment px-4 py-3">
          ...
          <div className="mt-3 flex items-center gap-4 text-[11px] font-medium">
            <button onClick={() => setLiked(!liked)}
              className={`flex items-center gap-1 transition-colors ${liked ? "text-scent-oudRose" : "text-loreal-muted hover:text-scent-noir"}`}>
              <span className="text-sm">{liked ? "♥" : "♡"}</span>
              <span>{post.likes + (liked ? 1 : 0)}</span>
            </button>
            <button className="flex items-center gap-1 text-loreal-muted hover:text-scent-noir transition-colors">
              <span className="text-sm">◌</span>
              <span>{post.comments}</span>
            </button>
            <button onClick={() => setSaved(!saved)}
              className={`flex items-center gap-1 transition-colors ${saved ? "text-scent-gold" : "text-loreal-muted hover:text-scent-noir"}`}>
              <span className="text-sm">{saved ? "◆" : "◇"}</span>
              <span>{post.saves + (saved ? 1 : 0)}</span>
            </button>
            <span className="ml-auto text-loreal-muted text-[10px]">↪</span>
          </div>
        </div>
      </div>
    </FadeUp>
